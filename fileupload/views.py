...
import os
import time
from login.views.function import check_filename_available
...
def upload_index(request):
    if request.method == 'POST':
        ...
        with open(os.path.join(MEDIA_ROOT,'upload', str(filename)), 'wb') as f:
...
def upload_complete(request):
    ...
    filepath = check_filename_available(os.path.join(MEDIA_ROOT,'upload',str(target_filename)))
    with open(filepath, 'wb') as target_file:  # 创建新文件
        while True:
            try:
                filename = os.path.join(MEDIA_ROOT,'upload',str(task)+str(chunk))
