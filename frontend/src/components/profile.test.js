...
import WrappedProfileForm from "./profile";

// jest.mock('./WrappedProfileForm');
let div;
...
test("and so is a spec", function () {
  let panel;
  ReactTestUtils.act(() => {
    // WrappedProfileForm.props.mockResolvedValue({ location: { state: {username: "", password: ""} } });
    panel = ReactDOM.render(<HashRouter><WrappedProfileForm /></HashRouter>, div);
